...
def check_perceptron(tracker):
    ...
    for dimensions in range(1, 10):
        p = PerceptronModel.PerceptronModel(dimensions)
    ...
    model = PerceptronModel.PerceptronModel(3)
...
def check_digit_classification(tracker):
    import DigitClassification
    model = DigitClassification.DigitClassificationModel()
...
def check_lang_id(tracker):
    import DigitClassification
    model = DigitClassification.LanguageIDModel()
...
def check_convolution(tracker):
    ...
    model = DigitClassification.DigitConvolutionalModel()
    ...
    for matrix_size in (2, 4, 6): #Test 3 random convolutions to test convolve() function
        ...
        student_output = DigitClassification.Convolve(torch.Tensor(input), torch.Tensor(weights))
...
def check_attention(tracker):
    ...
    for block_size in [2,4,16]:
        ...
        att_block = DigitClassification.Attention(layer_size, block_size)
